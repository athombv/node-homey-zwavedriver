...
module.exports = payload => {
  ...
  const precision = properties1['Size'] || properties2['Precision'];

  const meterValue = payload['Meter Value'];
  const previousMeterValue = payload['Previous Meter Value'];

  // Add Meter Value (Parsed)
  if (Buffer.isBuffer(meterValue) && meterValue.length === size && precision) {
  ...
  if (Buffer.isBuffer(previousMeterValue) && previousMeterValue.length === size && precision) {
