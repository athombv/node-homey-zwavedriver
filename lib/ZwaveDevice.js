...
class ZwaveDevice extends Homey.Device {
  ...
  _composeCustomSaveMessage(oldSettings, newSettings, changedKeysArr) {
    ...
    if (this.node.battery === true && this.node.online === false) saveMessage = this.zwavedriverI18n('settings.offlineNodeSaveMessage');
  ...
  async _getCapabilityValue(capabilityId, commandClassId) {
    ...
    try {
      ...
      this.error('Error: capability get command failed', {capabilityId, commandClassId, parsedPayload}, err);
      throw err;
    }
  }
