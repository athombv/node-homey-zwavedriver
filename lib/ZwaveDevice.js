...
class ZwaveDevice extends Homey.Device {
  ...
  _registerCapabilityGet(capabilityId, commandClassId) {
    ...
    if (capabilityGetObj.opts.getOnStart) {
      ...
      if (this.node.battery === false)
        this._getCapabilityValue(capabilityId, commandClassId).catch(err =>
          this.error(`Could not get capability value for capabilityId: ${capabilityId}`, err),
        );
  ...
  async _getCapabilityValue(capabilityId, commandClassId) {
    ...
    const commandClass =
      capabilityGetObj.node.CommandClass[`COMMAND_CLASS_${capabilityGetObj.commandClassId}`];
  ...
  async _setCapabilityValue(capabilityId, commandClassId, value, opts = {}) {
    ...
    const commandClass =
      capabilitySetObj.node.CommandClass[`COMMAND_CLASS_${capabilitySetObj.commandClassId}`];
  ...
  _registerCapabilityRealtime(capabilityId, commandClassId) {
    ...
    const commandClass =
      capabilityReportObj.node.CommandClass[`COMMAND_CLASS_${capabilityReportObj.commandClassId}`];
    if (typeof commandClass === 'undefined')
      return this.error('Invalid commandClass:', capabilityReportObj.commandClassId);
  ...
  _getCapabilityObj(commandType, capabilityId, commandClassId) {
    ...
    if (typeof commandClassId !== 'undefined') {
      ...
      for (const commandClassId in capability) {
        if (Object.prototype.hasOwnProperty.call(capability, commandClassId)) {
          commandClass = capability[commandClassId];
        }
      }
    }
   ...
   * @param {Object} [userOpts] - The object with options for this capability/commandclass combination. These will extend system options, if available (`/system/`)
   ...
   * @param {Number} [userOpts.getOpts.pollMultiplication] - Multiplication factor for the pollInterval key, must be a number. (e.g. 1000 to convert to seconds, 60.000 for minutes, 3600000 for hours)
   ...
   * @param {Function} [userOpts.fn] - This function is called after a setCapabilityValue has been called
   ...
   * @param {Boolean} [userOpts.reportParserOverride] - Boolean flag to determine if the `reportParser` method should override all report parsers. (Assumed false when not specified)
   ...
   * @param {Number} [userOpts.multiChannelNodeId] - An ID to use a MultiChannel Node for this capability
   * @param {Function} [userOpts.fn] - This function is called after a setCapabilityValue has been executed
   * @param {Object} [userOpts.fn.value] - The capability value
   */
  registerCapability(capabilityId, commandClassId, userOpts) {
    ...
    if (userOpts) {
      userOpts.reportParserOverride =
        typeof userOpts.reportParser === 'function' && userOpts.reportParserOverride === true;
    }
  ...
  async configurationSet(options = {}, value) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const result = await commandClassConfiguration.CONFIGURATION_SET({
        ...
        Object.keys(this.node.MultiChannelNodes[multiChannelNodeId].CommandClass).forEach(
          commandClassId => {
            this.node.MultiChannelNodes[multiChannelNodeId].CommandClass[commandClassId].on(
              ...
              function () {
                this.log(
                  `node.MultiChannelNodes['${multiChannelNodeId}'].
          CommandClass['${commandClassId}'].on('report')`,
                  'arguments:',
                  arguments,
                );
              }.bind(this),
