				...
				optionsCapability.forEach((optionsCapabilityItem) => {
					...
					if (optionsCapabilityItem.command_get && instance.CommandClass[optionsCapabilityItem.command_class]) {
						...
						if (optionsCapabilityItem.getOnWakeUp) {
							if (zwaveNode && zwaveNode.hasOwnProperty("CommandClass") &&
							zwaveNode.CommandClass.hasOwnProperty("COMMAND_CLASS_WAKE_UP")) {
								zwaveNode.CommandClass['COMMAND_CLASS_WAKE_UP'].on('report', command => {
									if (command && command.name === "WAKE_UP_NOTIFICATION") get.call(this);
								});
							}
						}
