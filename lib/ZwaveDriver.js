...
class ZwaveDriver extends events.EventEmitter {
		...
		Homey.wireless('zwave').getNode( device_data, ( err, node ) => {
			...
			this.driver.capabilities.forEach((capability_id) => {
				...
				options_capability.forEach((options_capability_item) => {
					...
					// subscribe for configuration reports
					instance.CommandClass['COMMAND_CLASS_CONFIGURATION'].on('report', ( command, report ) => {
						this.driver.settings.forEach((settings_id) => {
							if (this.options.settings[settings_id.id] != 'undefined') {
								if (this.options.settings[settings_id.id]['index'] == report['Parameter Number']) {
									var new_setting = {}
									new_setting[settings_id.id] = JSON.parse(JSON.stringify(report['Configuration Value'])).data[0];
									this._debug('Option found:', settings_id.id, 'Parameter id:', report['Parameter Number'], 'Value:', new_setting[settings_id.id], 'Driver:', this.options.driver_id);
									Homey.manager("drivers").getDriver(this.options.driver_id).setSettings(device_data, new_setting, function(err, settings) {
	...
	deleteNode( device_data ) {
		...
		// unregister configuration report
		this.nodes[ device_data.token ].instance.CommandClass[ 'COMMAND_CLASS_CONFIGURATION' ].removeAllListeners('report');
