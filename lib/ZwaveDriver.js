	...
	initNode(deviceData, callback) {
		...
		Homey.wireless('zwave').getNode(deviceData, (err, zwaveNode) => {
			...
			if (this.options.debug === true) {
				...
				nodeEvents.forEach((nodeEvent) => {
					zwaveNode.on(nodeEvent, (event) => {
						this._debug(`node.on('${nodeEvent}')`, 'arguments:', { 'event': event });
					});
				});

				Object.keys(zwaveNode.CommandClass).forEach(commandClassId => {
					zwaveNode.CommandClass[commandClassId].on('report', (command, report) => {
						this._debug(`node.CommandClass['${commandClassId}'].on('report')`, 'arguments:', [ { 'command': command }, { 'report': report } ]);
					});
				});
				...
				if (zwaveNode.MultiChannelNodes) {
					Object.keys(zwaveNode.MultiChannelNodes).forEach(multiChannelNodeId => {
						Object.keys(zwaveNode.MultiChannelNodes[multiChannelNodeId].CommandClass).forEach(commandClassId => {
							zwaveNode.MultiChannelNodes[multiChannelNodeId].CommandClass[commandClassId].on('report', (command, report) => {
								this._debug(`node.MultiChannelNodes['${multiChannelNodeId}'].
								CommandClass['${commandClassId}'].on('report')`, 'arguments:', [ { 'command': command }, { 'report': report } ]);
							});
						});
					});
				}
