...
class ZwaveDriver extends events.EventEmitter {
	...
	initNode( device_data, callback ) {
		...
		Homey.wireless('zwave').getNode( device_data, ( err, node ) => {
			...
			if(instance.CommandClass['COMMAND_CLASS_CONFIGURATION']){
					...
					this.driver.settings.forEach((settings_id) => {
						if (this.options.settings[settings_id.id] != 'undefined') {
							if (this.options.settings[settings_id.id]['index'] == report['Parameter Number']) {
								...
								this._debug('Option found:', settings_id.id, 'Parameter id:', report['Parameter Number'], 'Value:', new_setting[settings_id.id]);
								this.setSettings(device_data, new_setting, function(err, settings) {
									if (err) return console.error(err);
								})
							}
						}
